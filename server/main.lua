...
local PlayersHarvestingEcstasy, PlayersTransformingEcstasy, PlayersSellingEcstasy = {}. {}, {}
...
local function HarvestEcstasy(source)
	...
	SetTimeout(5000, function()
		if PlayersHarvestingEcstasy[source] == true then
			...
			if xPlayer.canCarryItem('ecstasy_brick', 1) then
				xPlayer.showNotification(_U('inv_full_ecstasy'))
			elseif money < 1000 then
				xPlayer.showNotification(_U('need_more_money'))
            else
	...
	SetTimeout(10000, function()
		if PlayersTransformingEcstasy[source] == true then
			...
			if ecstasy_pillQTE > 350 then
				xPlayer.showNotification(_U('too_many_pills'))
			elseif ecstasy_brickQTE < 1 then
				xPlayer.showNotification(_U('not_enough_bricks'))
			else
...
AddEventHandler('esx_ecstasy:GetUserInventory', function(currentZone)
    ...
    TriggerClientEvent('esx_ecstasy:ReturnInventory', _source, xPlayer.getInventoryItem('ecstasy_brick').count, xPlayer.getInventoryItem('ecstasy_pills').count, xPlayer.job.name, currentZone)
end)
